...
| 9 | Every row is accounted for | **PARTIAL** | `cd apps/api && python -m pytest tests/test_tombstone_polarity.py tests/test_row_conservation.py tests/test_property9_row_conservation.py tests/test_migration_certificate.py tests/test_transfer_mirror.py -q` (73 passed in 5.26s on this host). Frontend: `npx tsx --test src/lib/conservationLedger.test.ts src/lib/transferConstants.test.ts` (22 passed); `npm run build` tsc+vite clean | Overwrite: dest COUNT(*). Keyed/CDC: dest-engine `dest_delta == inserts - deletes` on **keys**, not at-least-once events. Mirror: `COUNT(*) WHERE NOT _deleted`. Writer ack never closes. | Inferred deletes on upsert/CDC without tombstone and not mirror; stream-path this-run `soft_deleted` census; Oracle/SQL Server live COUNT; dest-only / file-export; multi-table job rollup; exactly-once |
