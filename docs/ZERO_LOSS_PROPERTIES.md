...
70 passed in 5.22s
  ...
  Lakehouse / object-store dest-before (this host, after 2026-08-14 slice):
    ...
    Object-store Gate-8 checksum (this host, after 2026-08-14 slice):
      334 passed, 9 skipped in 39.77s (Property 5 + 9 + ADLS/SFTP Gate-8 + Excel phantom).
      ...
      `{"records":[...]}`: COUNT=3, checksum unmeasured. XML: COUNT=2,
      checksum unmeasured. Parquet value walk n=2 / garbage unmeasured.
      ...
      XML unique-path COUNT=2, checksum still unmeasured (one-shot GET).
...
* Object-store Gate-8 cell checksum — **PARTIAL**: `checksum_object_store`
  ...
  host. XML unique-path stays checksum-unmeasured on a one-shot GET.
