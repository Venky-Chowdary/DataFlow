...
70 passed in 5.22s
  ...
  Lakehouse / object-store dest-before (this host, after 2026-08-14 slice):
    ...
    Object-store Gate-8 checksum (this host, after 2026-08-14 slice):
      334 passed, 9 skipped in 40.05s (Property 5 + 9 + ADLS/SFTP Gate-8 + Excel phantom).
      ...
      `{"records":[...]}`: COUNT=3, checksum unmeasured. XML unique-path
...
* Object-store Gate-8 cell checksum — **PARTIAL**: `checksum_object_store`
  ...
  Empty `[]` is `(0, "")`. Wrapped JSON keeps COUNT and stays
  checksum-unmeasured on a one-shot GET. XML unique-path cell dicts are
  a second StAX pass of the COUNT path (one-shot GET is spooled; empty
  `(0, "")`; nested inner lists lose to the outer record; ambiguous
  siblings / document XML stay unmeasured). Parquet GET checksum is a
