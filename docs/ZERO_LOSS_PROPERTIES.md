...
9. File/object export: `count_artifact_rows` re-opens the written file
   ...
   Local CSV/JSON/JSONL/XML gzip streams (`open_artifact_binary`); object-store
...
10. Lakehouse / object-store dest-before: `destination_row_count` for
    ...
    CSV/TSV RFC 4180 records). GET of CSV/JSON/JSONL/XML walks the HTTP
    ...
    Excel/Avro/Parquet/ORC GET still materializes one object. JSON-parse
...
70 passed in 5.22s
  ...
  Lakehouse / object-store dest-before (this host, after 2026-08-14 slice):
    ...
    Object-store Gate-8 checksum (this host, after 2026-08-14 slice):
      335 passed, 9 skipped in 40.32s (Property 5 + 9 + ADLS/SFTP Gate-8 + Excel phantom).
  ...
  Iceberg catalog dest COUNT + SqlCatalog leftover MERGE (this host, after 2026-08-14 slice):
    ...
    Dest COUNT is `len(_iceberg_snapshot_rows)`, never pyiceberg
    `scan().count()` / manifest `record-count`. Missing SqlCatalog
    ...
    Gate-8 Iceberg count is `len` of the Arrow scan it already holds.
    MoR / deletion vectors Planned. Frontend 29 passed; `npm run build`
    tsc+vite clean.
  ...
  Gzip artifact dest COUNT stream (this host, after 2026-08-14 slice):
    ...
    GET gzip of CSV/JSON/JSONL/XML now streams (next paragraph).
    Cardinality ≠ Gate-8. `engine.py` unchanged. Frontend not run this
    slice (no UI change).
  ...
  CSV prefix-then-rest one-shot gzip COUNT (this host, after 2026-08-14 slice):
    ...
    quoted-newline 2; JSONL/JSON/XML one-shot gzip 2/3/2; corrupt
  ...
  Object-store GET stream dest COUNT (this host, after 2026-08-14 slice):
    ...
    Excel/Avro/Parquet/ORC still materialize one object. Live moto/MinIO
...
* Iceberg dest-before — **PARTIAL** on filesystem CoW: missing=0, COUNT=2,
  ...
  dest COUNT = `len` of snapshot rows, never `scan().count()`). Missing
