...
70 passed in 5.22s
  ...
  ClickHouse dest COUNT FINAL (this host, after 2026-08-14 slice):
    ...
    overwrite leftover MERGE stays `None` (mutations async — not in
    `warehouse_sql_quote_dialect`). Reuses `clickhouse_final_table_sql`
    (writer / Gate-8 fingerprint). Live ClickHouse cluster not on this
    host. Frontend 29 passed; `npm run build` tsc+vite clean.
...
* Oracle / SQL Server dest COUNT + leftover MERGE listing — algorithm in
  ...
  `system.tables.total_rows`); leftover MERGE stays unapplied (mutations
  async). Live ClickHouse skip-if-unreachable.
