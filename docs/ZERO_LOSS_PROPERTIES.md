...
| 9 | Every row is accounted for | **PARTIAL** | `cd apps/api && python -m pytest tests/test_tombstone_polarity.py tests/test_row_conservation.py tests/test_property9_row_conservation.py tests/test_migration_certificate.py tests/test_transfer_mirror.py tests/test_non_cdc_multistream_sequential.py tests/test_stream_append_precount.py tests/test_execute_tracked_sqlite_to_csv_to_sqlite_roundtrip.py tests/test_milvus_writer.py tests/test_qdrant_writer.py tests/test_enterprise_hardening.py -q` (counts filled after this-host run). Frontend: `npx tsx --test src/lib/conservationLedger.test.ts src/lib/transferConstants.test.ts`; `npm run build` tsc+vite | Overwrite: dest COUNT(*). Complete PK census splits MISSING_TARGET vs EXTRA_TARGET. Complete overwrite snapshot MERGE-deletes dest keys not in S (`leftover = D \\ S`) before Gate-8 COUNT — incremental CDC is a hard no-op. Keyed/CDC: dest-engine `dest_delta == inserts - deletes` on **keys**. Dest-before before first write. Mirror: `COUNT(*) WHERE NOT _deleted`. Job closed iff every stream closed. File/object export: independent artifact record COUNT. Vector/RAG: `COUNT(DISTINCT source_id)` (pgvector SQL, Milvus entity query, Qdrant point scroll) — never chunk COUNT(*) / collection rowCount / writer ack. SCD2: `COUNT(*) WHERE is_current`. Writer ack never closes. | Inferred deletes on incremental upsert/CDC without tombstone; stream-path this-run `soft_deleted` census; Oracle/SQL Server live COUNT; Pinecone/Weaviate DISTINCT source_id; live moto/MinIO object-store COUNT; live shared-reader CDC dest-before on PG logical; Iceberg leftover MERGE; exactly-once |
...
70 passed in 5.22s
  ...
  Dest-only vector identity (this host, after 2026-08-14 slice):
    ...
    Unit: 5 chunks / 2 source_ids close as dest 2; missing collection = 0;
      ...
      unreachable). Pinecone/Weaviate stay dest_only writer_ack until
      DISTINCT exists. Live Qdrant certify when localhost:6333 is up;
      live Milvus skipped unless reachable.
...
* dest-only sinks besides dest-engine DISTINCT `source_id`: Pinecone /
  ...
  closes Qdrant on writer ack). Live Milvus certify skipped unless the
  cluster is reachable; live Qdrant certify when localhost:6333 is up.
