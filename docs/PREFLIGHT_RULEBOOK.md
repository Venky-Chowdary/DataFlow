...
| **Constraint hints** | Relational FK awareness (Studio / host policy — **not** a `GateId`) | Soft / informational | Destination foreign-key columns are undeclared in the mapping or missing from the schema snapshot. | Map the FK columns, confirm parent rows exist, or defer until live FK introspection is wired. Hints attach as `constraint_hints` on the Validate result and **never** flip `passed`. | Mapped table has `customer_id` FK → `customers.id` but `customer_id` is unmapped. |
