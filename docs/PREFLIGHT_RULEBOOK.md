...
| **G10 Schema Policy** | Schema policy (Studio / host policy — **not** in `GateId`) | Soft | The selected schema policy is conservative or the target schema is unknown. | Switch to `propagate` or `auto_create` if the target is allowed to evolve. | Manual-review policy prevents auto-create. |
| **G11 Validation Posture** | Validation posture (Studio / host policy — **not** in `GateId`) | Soft | The validation mode is strict and the user may want more leniency. | Change to `balanced` or `maximum` only if the transfer is non-critical and data quality is low. | Strict mode rejects moderate-confidence mappings. |
| **Constraint findings** | Relational FK awareness + optional sample orphan probe (Studio / host policy — **not** a `GateId`) | Hard when severity=`block` (strict/maximum, unmapped dest FK or sample orphans, no ack) | Destination foreign-key columns are undeclared in the mapping, or Validate-sample FK values lack parents. | Map the FK columns, fix orphans, confirm parent rows exist, or acknowledge FK risk with an audit trail. Findings attach as `constraint_findings`; blocking findings also add `constraint_fk` and flip `passed=false`. **Sample orphan probe ≠ population RI proof.** | Mapped table has `customer_id` FK → `customers.id` but `customer_id` is unmapped; or sample has `customer_id=99` missing from `customers`. |

**Required core gates in `packages/preflight` (`GateId` / `PREFLIGHT_GATES`) are G1–G9 only.** Host policy rows above (schema policy, validation posture, constraint hints) are optional Studio extras — do not market constraint hints as a numbered product gate.
...
- Backend implementation references: `apps/api/services/preflight_rules.py`, `apps/api/services/preflight_service.py`, `apps/api/services/validation_plan.py`, `packages/preflight/src/preflight/gates.py`, `packages/preflight/src/preflight/constraint_hints.py`, `apps/api/services/error_handling.py`, `apps/api/services/lineage_telemetry.py`.
