...
| **Constraint findings** | Relational FK awareness (Studio / host policy — **not** a `GateId`) | Hard when severity=`block` (strict/maximum, unmapped dest FK, no ack) | Destination foreign-key columns are undeclared in the mapping. | Map the FK columns, confirm parent rows exist, or acknowledge FK risk with an audit trail. Findings attach as `constraint_findings` / `constraint_hints`; blocking findings also add `constraint_fk` and flip `passed=false`. **Coverage = destination FK metadata only — never claims population orphan / RI proven.** | Mapped table has `customer_id` FK → `customers.id` but `customer_id` is unmapped. |
