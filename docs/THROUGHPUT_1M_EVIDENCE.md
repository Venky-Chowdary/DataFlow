...
### Named 1M fixture — Iceberg↔Mongo / Iceberg↔Iceberg / SQL Server↔Mongo / Oracle↔Mongo / SQLite identity / MinIO S3 identity / SQLite↔Mongo / SQLite↔MySQL / SQLite↔Iceberg / SQLite↔SQL Server / SQLite↔Oracle / S3↔Iceberg / SQL Server↔S3 / Oracle↔S3 / GCS identity / DynamoDB identity / Snowflake identity — wired, not measured
...
`bench_snowflake_to_snowflake_million.py`, plus SQL
...
`COUNT(*)` (SQLite / PostgreSQL / Snowflake), object-store artifact COUNT (S3 —
...
writer ack). Iceberg→Iceberg writes **new** dest
...
account+database+schema+table declines. Cross-account Snowflake INSERT SELECT declines. JSON
