...
BENCH_ROWS=1000000 BENCH_SRC=bench_sqlite_oracle BENCH_DEST=bench_oracle_s3.csv \\
...
### Named 1M fixture — MySQL→Iceberg SELECT + CSV + snapshot (2026-09-03)

MySQL has no `COPY TO STDOUT`. Identity bulk is one
`START TRANSACTION WITH CONSISTENT SNAPSHOT` + SSCursor `SELECT`
encoded as CSV into one Arrow table and one Iceberg catalog snapshot.
...
Do not mix this 9.219 s with PostgreSQL→Iceberg 1.344 s: PostgreSQL
emits CSV in the server; MySQL pays a Python CSV encode. Do not mix
either with network SQL engine times.

Reproduce: `BENCH_SRC=bench_1m BENCH_DEST=bench_mysql_iceberg python scripts/bench_mysql_to_iceberg_million.py`

| Item | Value |
|------|-------|
| Source | MySQL **3306** (`bench_1m`, 10 columns, PK `employee_id`) |
| Destination | Iceberg REST **8181**, create-new `default.bench_mysql_iceberg` |
| Warehouse | `file:///tmp/iceberg-rest-wh` (local files, not S3) |
| Rows | **1,000,000** |
| Elapsed | **9.219 s** |
| dest COUNT (file footers) | **1,000,000** |
...
| `iceberg_write` | `append` |
| `shard_mode` | `table` |
| Artifact | `/opt/cursor/artifacts/mysql_iceberg_1000000_proof.json` |
...
| `copy_split` | skip |
| `iceberg_write` | skip |
| Elapsed | **0.683 s** (COUNTs only — not a copy-speed figure) |
...
### Named 1M fixture — Iceberg↔Mongo / Iceberg↔Iceberg / SQL Server↔Mongo / Oracle↔Mongo / SQLite identity / MinIO S3 identity / SQLite↔Mongo / SQLite↔MySQL / SQLite↔Iceberg / SQLite↔SQL Server / SQLite↔Oracle / S3↔Iceberg / SQL Server↔S3 / Oracle↔S3 / GCS identity / Elasticsearch identity — wired, not measured
...
`bench_gcs_to_gcs_million.py`, `bench_elasticsearch_to_elasticsearch_million.py`, plus SQL
...
(PostgreSQL→S3 / MySQL→S3 / SQLite→S3 / Mongo→S3 / Iceberg→S3 / SQL Server→S3 / Oracle→S3), not `aws s3 cp` / `aws s3 sync` / GET+PUT.
...
is `insert_many`, not `mongoimport`. SQLite→Mongo empty dest is
`insert_many`, not `mongoimport` / `.dump`. Mongo→SQLite empty dest is
...
sqlldr / `.import`. S3→Iceberg empty dest is GET CSV + CoW snapshot
append, not `aws s3 cp` / `MERGE INTO`. Iceberg→S3 empty dest is snapshot
Parquet + `upload_file`, not `MERGE INTO` / `aws s3 cp`. SQL Server→S3
empty dest is HOLDLOCK SELECT CSV + `upload_file`, not BCP / `aws s3 cp`.
S3→SQL Server empty dest is GET CSV + `fast_executemany`, not BCP /
...
Azurite, not a customer-tenant PRODUCTION_SKU). Elasticsearch→Elasticsearch empty dest is
cluster `_reindex`, not scroll+bulk / `helpers.reindex` / `_cat/indices`
`docs.count` (this host's proof is desktop-lab Elasticsearch
`127.0.0.1:9200`; dest COUNT is `_count` after refresh via
`destination_row_count`). Iceberg→Iceberg writes **new** dest
...
endpoint+container+blob declines. Cross-endpoint ADLS start_copy_from_url declines. Elasticsearch same
host+port+index declines. Cross-endpoint Elasticsearch `_reindex` declines. JSON
...
Iceberg→Iceberg, Iceberg→SQLite, and Iceberg→S3. SQLite DATE affinity declines SQLite→PostgreSQL (would
...
SQLite→SQL Server, and SQLite→Oracle. SQLite / S3 TEXT identity on Oracle is
