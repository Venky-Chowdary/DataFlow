...
`COUNT(*)` (SQLite / PostgreSQL / Snowflake), or object-store artifact COUNT (S3 —
...
Azurite, not a customer-tenant PRODUCTION_SKU). Snowflake→Snowflake empty dest is
...
endpoint+container+blob declines. Cross-endpoint ADLS start_copy_from_url declines. Snowflake same
