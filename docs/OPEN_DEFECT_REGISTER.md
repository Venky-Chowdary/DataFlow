...
| D6 | PostgreSQL destination key census fails with `operator does not exist: text = integer` — the census binds the key literal untyped against a typed column. | key census in `services/dest_precount.py` | C | named open in handover §4 |
...
| D8 | `type_system` reads the bare token `long` as Oracle `LONG` (text), so a document-store `long → BIGINT` is flagged as invention. | `services/type_system.py` | E | handover §4 |
