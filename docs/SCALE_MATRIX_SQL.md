...
| 18 | * → sqlite | `uid UUID → TEXT` / `→ VARCHAR`, `CHAR(36) → TEXT` "collapse fidelity" | SQLite has no UUID/bounded-string type; TEXT is *wider*, not narrower, so this should be a carrier note, not a fail-closed collapse. Needs a SQLite carrier-equivalence rule in the fidelity gate. |
