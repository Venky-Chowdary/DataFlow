  ...
  "help-connectors": {
    ...
    sections: [
      ...
      },
      {
        ...
        steps: [
          "**Certified** — governed transfer path proven (`transfer_ready`); safe for production schedules after Test passed.",
          "**Source only** — readable; no transfer-ready write path.",
          "**Test only** — auth works; write path is not transfer-ready.",
      ...
      },
      {
        ...
        body: "**PostgreSQL**, **MySQL**, and **MongoDB** are certified full transfer. **Snowflake**, **BigQuery**, **S3**, **CSV**, and **JSON** are TRANSFER_READY when their packages are installed. **Redshift** stays Planned until a named PRODUCTION_SKU matrix. Generic **SQLAlchemy** URLs extend reach with the same **Validate** preflight path used by native drivers. Catalog tiles are not transfer-live. Proofs → Integrity ledger has **Run desktop lab**: 45 catalog slots exercised as source and destination when the desktop backend is up. Hosted twins (Neon/RDS/CNPG/OpenShift PostgreSQL) share a driver — that option is real, it is not 45 unique engines.",
      },
    ],
  },
  "help-pipelines": {
