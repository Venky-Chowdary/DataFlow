...
/**
 * Shaping sits before Map deliberately: Map decides carriers, narrowing risk
 * contracts and the DDL identity from the columns and values it is shown, so
 * shaping must be the last thing that changes source-side truth.
 */
...
export const STEPS: { n: number; label: string; shortLabel: string; icon: string }[] = [
  ...
  { n: STEP_SHAPE, label: "Shape", shortLabel: "Shape", icon: "layers" },
