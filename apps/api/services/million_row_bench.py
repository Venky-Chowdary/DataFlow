...
def run_sqlite_pg_volume(
    *,
    ...
    not upsert / ``.dump``. BLOB/unix DATETIME/boolean synonyms on the
    SQLite source decline. Naive ISO DATE/DATETIME and BOOLEAN 0/1 copy.
    Seeds from PostgreSQL→SQLite when the source file/table is missing —
    hire_date is TEXT after that hop.
    """
    import socket
...
def run_oracle_s3_volume(
    *,
    ...
    """Identity Oracle→S3 through stream_database_transfer.

    Dest COUNT is object-store artifact COUNT of the CSV (header skipped),
    never ListObjects length, never writer PUT ack. Empty dest is SHARE-
    lock SELECT CSV + ``upload_file``, not sqlldr / Data Pump /
    ``aws s3 cp``. Dest key must be ``.csv`` / ``.tsv``. BLOB/CLOB/JSON
    decline. Unique dest ``bench_oracle_s3.csv`` is not reused from
    ``bench_mysql_s3.csv`` / ``bench_pg_s3.csv`` / ``bench_sqlite_s3.csv``
    / ``bench_sqlserver_s3.csv``. Seeds from SQLite→Oracle when the
    ...
    src_table = str(source_table or "bench_sqlite_oracle")
    dest = str(dest_table)
    bucket = dest_bucket or _s3_bench_bucket()
    dest_cfg = _s3_local_cfg(bucket, dest)
    job_store = ensure_memory_job_store_if_mongo_down()
    try:
        have = _oracle_dest_count(src_table)
    except Exception:
        have = 0
    if have != rows:
        run_sqlite_oracle_volume(
    ...
    schema = dict(cols)
    job_id = f"bench-oracle-s3-{dest}-{int(time.time())}"
    get_mongodb_service().create_transfer_job({"_id": job_id, "name": job_id})
    ...
    print(
        f"\n=== {bucket}/{dest} oracle→s3 [{sync_mode}]: {transferred} rows "
        f"in {elapsed:.1f}s = {rps:,.0f} rows/s"
    )
    for key in REPORTED_SUMMARY_KEYS:
        if key in summary:
    ...
    print(f"destination artifact COUNT: {landed} (source snapshot COUNT: {rows})")
    print(
        "Not sqlldr / Data Pump / aws s3 cp. Empty dest is SHARE-lock SELECT CSV + upload. "
        "CSV HEADER is not a dest row."
    )
    print(f"row conservation: {conservation['verdict']}")
    if proof_path:
    ...
    if fail_closed:
        ...
        expected_load = "select_oracle_upload_s3"
        if summary.get("load_method") != expected_load:
            raise AssertionError(
                "expected Oracle SHARE-lock SELECT CSV + S3 upload, "
                f"got load_method={summary.get('load_method')!r}"
            )
    return report


def run_s3_oracle_volume(
    *,
    rows: int,
    dest_table: str,
    source_table: str | None = None,
    ...
    """Identity S3→Oracle through stream_database_transfer.

    Source COUNT is object-store artifact COUNT of the CSV (header
    skipped). Dest COUNT is ``SELECT COUNT(*)``. Empty dest is GET CSV +
    ``executemany``, not sqlldr / Data Pump / ``aws s3 cp``.
    JSON/JSONL/Parquet decline. Unique dest ``bench_ora_from_s3`` is not
    reused from ``bench_1m`` / ``bench_sqlite_oracle``. Seeds from
    Oracle→S3 when the source object is missing. VARCHAR2 stores ``''``
    ...
    src_key = str(source_table or "bench_oracle_s3.csv")
    dest = str(dest_table)
    bucket = source_bucket or _s3_bench_bucket()
    src_cfg = _s3_local_cfg(bucket, src_key)
    job_store = ensure_memory_job_store_if_mongo_down()
    try:
        ...
        have = 0
    if have != rows:
        run_oracle_s3_volume(
            rows=rows,
            dest_table=src_key,
            dest_bucket=bucket,
            source_table="bench_sqlite_oracle",
            sync_mode="full_refresh_append",
            keep_dest=False,
            fail_closed=True,
            proof_path=None,
    ...
    schema = dict(cols)
    job_id = f"bench-s3-oracle-{dest}-{int(time.time())}"
    get_mongodb_service().create_transfer_job({"_id": job_id, "name": job_id})
    ...
    print(
        f"\n=== {dest} s3→oracle [{sync_mode}]: {transferred} rows "
        f"in {elapsed:.1f}s = {rps:,.0f} rows/s"
    )
    for key in REPORTED_SUMMARY_KEYS:
        if key in summary:
            print(f"{key}: {summary[key]}")
    print(f"destination COUNT(*): {landed} (source artifact COUNT: {rows})")
    print("Not sqlldr / Data Pump / aws s3 cp. Empty dest is GET CSV + executemany.")
    print(
        "empty_string_as_null_cells: "
        f"{summary.get('empty_string_as_null_cells') or 0}"
    )
    ...
    if fail_closed:
        ...
        expected_load = "get_csv_s3_executemany_oracle"
        if summary.get("load_method") != expected_load:
            raise AssertionError(
                "expected S3 GET CSV + Oracle executemany, "
                f"got load_method={summary.get('load_method')!r}"
            )
    return report


def _require_fake_gcs() -> None:
    import socket

    host = os.environ.get("FAKE_GCS_HOST", "127.0.0.1")
    port = int(os.environ.get("FAKE_GCS_PORT", "4443"))
    try:
        socket.create_connection((host, port), timeout=1).close()
    except OSError as exc:
        raise RuntimeError(f"fake-gcs {host}:{port} not reachable: {exc}") from exc


def _gcs_bench_bucket() -> str:
    return os.environ.get("BENCH_GCS_BUCKET", "dataflow-bench")


def _gcs_local_cfg(bucket: str, key: str) -> dict[str, Any]:
    host = os.environ.get("FAKE_GCS_HOST", "127.0.0.1")
    port = int(os.environ.get("FAKE_GCS_PORT", "4443"))
    return {
        "type": "gcs",
...
def _gcs_count(cfg: dict[str, Any], key: str) -> int:
    from services.copy_gcs_common import gcs_dest_count

    return gcs_dest_count(cfg, key)


def _gcs_delete_key(cfg: dict[str, Any], key: str) -> None:
    from services.copy_gcs_common import gcs_delete_keys, gcs_list_keys

    gcs_delete_keys(cfg, gcs_list_keys(cfg, key))


def _gcs_seed_jsonl(cfg: dict[str, Any], key: str, rows: int) -> None:
    """Source setup only — not the identity COPY path."""
    import tempfile

    from connectors.gcs_common import gcs_client, gcs_emulator_kwargs
    from services.copy_gcs_common import gcs_bucket, gcs_ensure_bucket

    gcs_ensure_bucket(cfg)
    fd, path = tempfile.mkstemp(prefix="df-gcs-seed-", suffix=".jsonl")
    os.close(fd)
    try:
        ...
        client.bucket(gcs_bucket(cfg)).blob(key).upload_from_filename(path, **kw)
    finally:
        try:
            os.unlink(path)
        except OSError:
...
def run_gcs_gcs_volume(
    *,
    ...
    """Identity GCS→GCS through stream_database_transfer.

    Dest COUNT is object-store artifact COUNT, never ListObjects length.
    Empty dest is server-side copy_blob / rewrite, not GET+PUT /
    ``gsutil cp``. Seeds JSONL upload when the source object is missing
    (seed is not the COPY path). Unique dest ``bench_gcs_clone.jsonl`` is
    not reused from ``bench_s3_clone.csv`` / ``bench_1m``. fake-gcs is
    not a customer-tenant PRODUCTION_SKU.
    """
    _require_fake_gcs()
    src_key = str(source_table or "bench_gcs_src.jsonl")
    dest = str(dest_table)
    bucket = dest_bucket or _gcs_bench_bucket()
    src_bucket = source_bucket or bucket
    src_cfg = _gcs_local_cfg(src_bucket, src_key)
    dest_cfg = _gcs_local_cfg(bucket, dest)
    if src_cfg["host"] == dest_cfg["host"] and src_bucket == bucket and src_key == dest:
        raise AssertionError("GCS→GCS bench refuses the same endpoint+bucket+object")
    job_store = ensure_memory_job_store_if_mongo_down()
    try:
        have = _gcs_count(src_cfg, src_key)
    except Exception:
        have = 0
    if have != rows:
        _gcs_seed_jsonl(src_cfg, src_key, rows)
    if not keep_dest:
        from services.copy_gcs_common import gcs_ensure_bucket

        gcs_ensure_bucket(dest_cfg)
        _gcs_delete_key(dest_cfg, dest)
    ...
    job_id = f"bench-gcs-gcs-{dest}-{int(time.time())}"
    get_mongodb_service().create_transfer_job({"_id": job_id, "name": job_id})
    ...
    elapsed = time.monotonic() - started
    landed = _gcs_count(dest_cfg, dest)
    rejected = int(summary.get("rejected_rows") or 0)
    conservation = row_conservation(
        source_rows=rows,
        dest_count=landed,
    ...
    report: dict[str, Any] = {
        ...
        "shard_mode": summary.get("shard_mode"),
        "copy_split": summary.get("copy_split"),
        "gcs_read": summary.get("gcs_read"),
        "gcs_write": summary.get("gcs_write"),
        "partition_proof": summary.get("partition_proof"),
        "proof_scope": summary.get("proof_scope"),
        "copy_workers": summary.get("copy_workers"),
        "copy_partitions": summary.get("copy_partitions"),
        ...
        "conservation": conservation,
        "columns": 2,
        "dest_count_source": "gcs_artifact_count",
        "summary_scalars": {
            key: summary[key] for key in REPORTED_SUMMARY_KEYS if key in summary
        },
    }
    print(
        f"\n=== {bucket}/{dest} gcs→gcs [{sync_mode}]: {transferred} rows "
        f"in {elapsed:.1f}s = {rps:,.0f} rows/s"
    )
    for key in REPORTED_SUMMARY_KEYS:
        if key in summary:
    ...
    print("Not gsutil cp / GET+PUT. Empty dest is copy_blob. fake-gcs is not PRODUCTION_SKU.")
    print(f"row conservation: {conservation['verdict']}")
    if proof_path:
        write_million_proof(proof_path, report)
    if fail_closed:
        ...
        expected_load = "copy_blob_gcs_gcs"
        if summary.get("load_method") != expected_load:
            raise AssertionError(
                "expected GCS copy_blob / rewrite, "
                f"got load_method={summary.get('load_method')!r}"
            )
    return report
...
def _require_elasticsearch() -> None:
    ...
    host = os.environ.get("DATAFLOW_ES_HOST", "127.0.0.1")
    port = int(os.environ.get("DATAFLOW_ES_PORT", "9200"))
    try:
        socket.create_connection((host, port), timeout=1).close()
    except OSError as exc:
        raise RuntimeError(f"Elasticsearch {host}:{port} not reachable: {exc}") from exc


def _es_local_cfg(index: str) -> dict[str, Any]:
    host = os.environ.get("DATAFLOW_ES_HOST", "127.0.0.1")
    port = int(os.environ.get("DATAFLOW_ES_PORT", "9200"))
    return {
        "type": "elasticsearch",
...
def _es_count(cfg: dict[str, Any], index: str) -> int:
    from services.copy_elasticsearch_common import elasticsearch_dest_count

    return elasticsearch_dest_count(cfg, index)


def _es_delete_index(cfg: dict[str, Any], index: str) -> None:
    from services.copy_elasticsearch_common import elasticsearch_delete_index

    elasticsearch_delete_index(cfg, index)


def _es_seed(cfg: dict[str, Any], index: str, rows: int) -> None:
    """Source setup only — not the identity COPY path."""
...
def run_elasticsearch_elasticsearch_volume(
    *,
    rows: int,
    dest_table: str,
    source_table: str | None = None,
    ...
    """Identity Elasticsearch→Elasticsearch through stream_database_transfer.

    Dest COUNT is cluster ``_count`` after refresh, never
    ``_cat/indices`` ``docs.count``. Empty dest is cluster ``_reindex``,
    not scroll+bulk. Seeds JSON bulk when the source index is missing
    (seed is not the COPY path). Unique dest ``bench_es_clone`` is not
    reused from ``bench_1m`` / ``bench_redis_clone`` /
    ``bench_gcs_clone.jsonl``. Desktop-lab Elasticsearch is not a
    customer-tenant PRODUCTION_SKU.
    """
    _require_elasticsearch()
    src_index = str(source_table or "bench_es_src")
    dest = str(dest_table)
    src_cfg = _es_local_cfg(src_index)
    dest_cfg = _es_local_cfg(dest)
    if (
        src_cfg["host"] == dest_cfg["host"]
        and src_cfg["port"] == dest_cfg["port"]
        and src_index == dest
    ):
        raise AssertionError("Elasticsearch→Elasticsearch bench refuses the same host+port+index")
    job_store = ensure_memory_job_store_if_mongo_down()
    try:
        have = _es_count(src_cfg, src_index)
    except Exception:
        have = 0
    if have != rows:
        _es_seed(src_cfg, src_index, rows)
    if not keep_dest:
        _es_delete_index(dest_cfg, dest)
    ...
    schema = {"id": "long", "label": "keyword"}
    job_id = f"bench-es-es-{dest}-{int(time.time())}"
    get_mongodb_service().create_transfer_job({"_id": job_id, "name": job_id})
    ...
    elapsed = time.monotonic() - started
    landed = _es_count(dest_cfg, dest)
    rejected = int(summary.get("rejected_rows") or 0)
    conservation = row_conservation(
        source_rows=rows,
        dest_count=landed,
    ...
    rps = transferred / elapsed if elapsed > 0 else 0.0
    report: dict[str, Any] = {
        "route": "elasticsearch→elasticsearch",
        "sync_mode": sync_mode,
        "source_table": src_index,
        "dest_table": dest,
        "job_store": job_store,
        "job_id": job_id,
        "load_method": summary.get("load_method"),
        "shard_mode": summary.get("shard_mode"),
        "copy_split": summary.get("copy_split"),
        "elasticsearch_read": summary.get("elasticsearch_read"),
        "elasticsearch_write": summary.get("elasticsearch_write"),
        "partition_proof": summary.get("partition_proof"),
        "proof_scope": summary.get("proof_scope"),
        "copy_workers": summary.get("copy_workers"),
        "copy_partitions": summary.get("copy_partitions"),
        ...
        "conservation": conservation,
        "columns": 2,
        "dest_count_source": "elasticsearch_count",
        "summary_scalars": {
            key: summary[key] for key in REPORTED_SUMMARY_KEYS if key in summary
        },
    }
    print(
        f"\n=== {dest} elasticsearch→elasticsearch [{sync_mode}]: {transferred} rows "
        f"in {elapsed:.1f}s = {rps:,.0f} rows/s"
    )
    for key in REPORTED_SUMMARY_KEYS:
        if key in summary:
            print(f"{key}: {summary[key]}")
    print(f"destination _count: {landed} (source _count: {rows})")
    print(
        "Not scroll+bulk / helpers.reindex / _cat docs.count. Empty dest is _reindex. "
        "Desktop-lab Elasticsearch is not PRODUCTION_SKU."
    )
    print(f"row conservation: {conservation['verdict']}")
    if proof_path:
        write_million_proof(proof_path, report)
    if fail_closed:
        ...
        expected_load = "reindex_elasticsearch_elasticsearch"
        if summary.get("load_method") != expected_load:
            raise AssertionError(
                "expected Elasticsearch _reindex, "
                f"got load_method={summary.get('load_method')!r}"
            )
    return report
