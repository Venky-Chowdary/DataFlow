...
def _wire_cell(value: object) -> str:
    if value is None:
        return "\\N"
    if isinstance(value, datetime):
        if value.tzinfo is not None:
            raise FastPathUnavailable("timestamptz MySQL value is not S3 COPY-safe")
...
def _format_row(cells: list[str], delimiter: str) -> str:
    if delimiter == "\t":
        return "\t".join(cells)
    ...
    *,
    ...
    try:
        ...
        with open(path, "w", encoding="utf-8", newline="\n") as handle:
            handle.write(_format_row(list(header), delimiter) + "\n")
            while True:
                ...
                for row in rows:
                    handle.write(
                        _format_row([_wire_cell(v) for v in row], delimiter) + "\n"
                    )
                    written += 1
        return written
    finally:
