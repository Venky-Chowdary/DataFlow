"""Oracle SELECT → PostgreSQL COPY FROM STDIN (cross-engine bulk).

The reverse of ``copy_pg_oracle``. This host has no client ``sqlldr`` /
Data Pump. One ``LOCK TABLE src IN SHARE MODE`` transaction streams
``SELECT``; each cell is encoded into PostgreSQL COPY text that
``COPY … FROM STDIN`` reads on the same thread (``read()`` fetches the
next Oracle batch). A FIFO + writer thread is **not** used: the same
deadlock class as SQL Server ``pyodbc`` + pipe was measured there.
...
COPY as ``\\N``. That is not a row drop.
...
from services.copy_sqlserver_pg import _SelectCopyReader

logger = logging.getLogger(__name__)

_UNSAFE_ORA_BASES = frozenset({
    "BLOB",
    "BFILE",
    "RAW",
    "LONG RAW",
...
def oracle_pg_copy_enabled() -> bool:
    raw = (getenv_brand("ORACLE_PG_COPY", "1") or "1").strip().lower()
    return raw not in {"0", "false", "no", "off"}


def oracle_type_is_copy_safe(declared: str) -> bool:
    ...
    return base not in _UNSAFE_ORA_BASES
...
def _select_sql(table_ref: str, source_cols: list[str], clause: str) -> str:
    cols = ", ".join(_ora_ident(c) for c in source_cols)
    where = f" WHERE {clause}" if clause and clause != "1=1" else ""
    return f"SELECT {cols} FROM {table_ref}{where}"  # nosec B608


def _tune_fetch(cur: Any) -> None:
    try:
        cur.arraysize = 8192
    except Exception:
        logger.debug("Oracle arraysize skipped", exc_info=True)
    try:
        cur.prefetchrows = 8192
    except Exception:
        logger.debug("Oracle prefetchrows skipped", exc_info=True)
...
def copy_oracle_to_postgres(
    *,
    ...
    """Stream Oracle rows into PostgreSQL COPY. Dest COUNT is the proof."""
    if not pairs or len(pairs) != len(pg_ddls):
        raise FastPathUnavailable("column list / DDL mismatch")
    if not oracle_pg_copy_enabled():
        raise FastPathUnavailable("Oracle→PostgreSQL COPY disabled")
    ...
    try:
        ...
        if dest_count != source_count:
            raise ValueError(
                "Oracle→PG COPY refused: dest COUNT(*) "
