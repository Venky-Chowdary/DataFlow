"""Time a real MySQL→MongoDB identity append through the production stream engine.
...
Not mongoimport. _id is not invented. MySQL has no COPY TO STDOUT —
one consistent-snapshot SELECT is bound with insert_many.

    cd apps/api && python scripts/bench_mysql_to_mongo_million.py
...
from services.million_row_bench import run_mysql_mongo_volume  # noqa: E402


if __name__ == "__main__":
    rows = int(os.environ.get("BENCH_ROWS", "1000000"))
    src = os.environ.get("BENCH_SRC", "bench_1m")
    dest = os.environ.get("BENCH_DEST", "bench_mysql_mongo")
    proof = os.environ.get("BENCH_PROOF")
    if not proof:
        proof = str(Path("/opt/cursor/artifacts") / f"mysql_mongo_{rows}_proof.json")
