"""Time a real MySQL→Oracle identity append through the production stream engine.
...
equals the source with rejected_rows=0. Oracle VARCHAR2 empty-string→NULL is
surfaced, not claimed as cell identity.

    cd apps/api && python scripts/bench_mysql_to_oracle_million.py
...
from services.million_row_bench import run_mysql_oracle_volume  # noqa: E402


if __name__ == "__main__":
    rows = int(os.environ.get("BENCH_ROWS", "1000000"))
    src = os.environ.get("BENCH_SRC", "bench_1m")
    dest = os.environ.get("BENCH_DEST", "BENCH_MY_ORA")
    proof = os.environ.get("BENCH_PROOF")
    if not proof:
        proof = str(Path("/opt/cursor/artifacts") / f"mysql_oracle_{rows}_proof.json")
