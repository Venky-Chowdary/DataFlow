"""Time a real MySQL→PostgreSQL append through the production stream engine.
...
Fails closed unless destination COUNT(*) equals the source with rejected_rows=0.
    ...
    cd apps/api && python scripts/bench_mysql_to_pg_million.py

    BENCH_ROWS=1000000 BENCH_SRC=bench_1m BENCH_DEST=bench_pg_from_mysql \\
      python scripts/bench_mysql_to_pg_million.py
"""
...
from services.million_row_bench import run_mysql_pg_volume  # noqa: E402
...
if __name__ == "__main__":
    ...
    dest = os.environ.get("BENCH_DEST", "bench_pg_from_mysql")
    proof = os.environ.get("BENCH_PROOF")
    if not proof:
        proof = str(Path("/opt/cursor/artifacts") / f"mysql_pg_{rows}_proof.json")
    run_mysql_pg_volume(
        rows=rows,
        source_table=src,
        dest_table=dest,
