"""Time a real PostgreSQL→Oracle identity append through the production stream engine.
...
destination COUNT(*) equals the source with rejected_rows=0.
    ...
    cd apps/api && python scripts/bench_pg_to_oracle_million.py
...
from services.million_row_bench import run_pg_oracle_volume  # noqa: E402


if __name__ == "__main__":
    rows = int(os.environ.get("BENCH_ROWS", "1000000"))
    src = os.environ.get("BENCH_SRC")
    dest = os.environ.get("BENCH_DEST", "BENCH_PG_ORA")
    proof = os.environ.get("BENCH_PROOF")
    if not proof:
        proof = str(Path("/opt/cursor/artifacts") / f"pg_oracle_{rows}_proof.json")
    run_pg_oracle_volume(
        rows=rows,
        dest_table=dest,
        source_table=src,
