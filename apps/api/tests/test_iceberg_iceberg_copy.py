"""Iceberg → Iceberg snapshot Parquet + CoW snapshot — dest footer COUNT."""
...
from services.copy_iceberg_iceberg import (  # noqa: E402
    copy_iceberg_to_iceberg,
    iceberg_iceberg_copy_enabled,
    iceberg_table_key,
)
...
def test_iceberg_iceberg_copy_safe_types():
...
def test_iceberg_iceberg_copy_kill_switch(monkeypatch):
    monkeypatch.setenv("DATAFLOW_ICEBERG_ICEBERG_COPY", "0")
    assert iceberg_iceberg_copy_enabled() is False
...
def test_iceberg_iceberg_same_table_declines(monkeypatch):
    monkeypatch.delenv("DATAFLOW_ICEBERG_ICEBERG_COPY", raising=False)
    cfg = _iceberg_cfg("same_table")
    assert iceberg_table_key(cfg) == iceberg_table_key(cfg)
...
@requires_rest
def test_live_iceberg_iceberg_dest_count(monkeypatch):
    pytest.importorskip("pyiceberg")
    pytest.importorskip("pyarrow")
    monkeypatch.delenv("DATAFLOW_ICEBERG_ICEBERG_COPY", raising=False)
    mysql = _mysql_connect()
    tag = uuid.uuid4().hex[:8]
    src = f"ice_ice_src_{tag}"
    mid = f"ice_ice_mid_{tag}"
    dest = f"ice_ice_dst_{tag}"
    try:
        _seed_iceberg_from_mysql(mysql, src, mid, 800)
        _drop_iceberg(dest)
        result = copy_iceberg_to_iceberg(
            source_cfg=_iceberg_cfg(mid),
            source_schema="default",
            source_table=mid,
...
@requires_rest
def test_live_iceberg_iceberg_empty_string_and_null_preserved():
    pytest.importorskip("pyiceberg")
    pytest.importorskip("pyarrow")
    from connectors.iceberg_catalog import load_catalog, parse_iceberg_catalog_config
    ...
    mysql = _mysql_connect()
    tag = uuid.uuid4().hex[:8]
    src = f"ice_ice_null_{tag}"
    mid = f"ice_ice_null_mid_{tag}"
    dest = f"ice_ice_null_dst_{tag}"
    try:
        with mysql.cursor() as cur:
            cur.execute(f"DROP TABLE IF EXISTS `{src}`")
            cur.execute(
...
@requires_rest
def test_live_iceberg_iceberg_skip_when_dest_count_matches():
    pytest.importorskip("pyiceberg")
    pytest.importorskip("pyarrow")
    mysql = _mysql_connect()
    tag = uuid.uuid4().hex[:8]
    src = f"ice_ice_skip_{tag}"
    mid = f"ice_ice_skip_mid_{tag}"
    dest = f"ice_ice_skip_dst_{tag}"
    try:
        _seed_iceberg_from_mysql(mysql, src, mid, 800)
        _drop_iceberg(dest)
        first = copy_iceberg_to_iceberg(
            source_cfg=_iceberg_cfg(mid),
            source_schema="default",
            source_table=mid,
            dest_cfg=_iceberg_cfg(dest),
        ...
        second = copy_iceberg_to_iceberg(
            source_cfg=_iceberg_cfg(mid),
            source_schema="default",
            source_table=mid,
            dest_cfg=_iceberg_cfg(dest),
...
@requires_rest
def test_live_iceberg_iceberg_occupied_mismatch_declines():
    pytest.importorskip("pyiceberg")
    pytest.importorskip("pyarrow")
    mysql = _mysql_connect()
    tag = uuid.uuid4().hex[:8]
    src = f"ice_ice_occ_{tag}"
    mid = f"ice_ice_occ_mid_{tag}"
    dest_src = f"ice_ice_occ_dsrc_{tag}"
    dest = f"ice_ice_occ_dst_{tag}"
    try:
        _seed_iceberg_from_mysql(mysql, src, mid, 800)
        _seed_iceberg_from_mysql(mysql, dest_src, dest, 2)
        ...
        with mysql.cursor() as cur:
            cur.execute(f"DROP TABLE IF EXISTS `{src}`")
            cur.execute(f"DROP TABLE IF EXISTS `{dest_src}`")
        _drop_iceberg(mid)
        _drop_iceberg(dest)
        mysql.close()
...
@requires_rest
def test_live_iceberg_iceberg_overwrite_replaces_snapshot():
    pytest.importorskip("pyiceberg")
    pytest.importorskip("pyarrow")
    mysql = _mysql_connect()
    tag = uuid.uuid4().hex[:8]
    src = f"ice_ice_ow_{tag}"
    mid = f"ice_ice_ow_mid_{tag}"
    dest_src = f"ice_ice_ow_dsrc_{tag}"
    dest = f"ice_ice_ow_dst_{tag}"
    try:
        _seed_iceberg_from_mysql(mysql, src, mid, 800)
        _seed_iceberg_from_mysql(mysql, dest_src, dest, 1)
        result = copy_iceberg_to_iceberg(
            source_cfg=_iceberg_cfg(mid),
            source_schema="default",
            source_table=mid,
...
@requires_rest
def test_live_iceberg_iceberg_source_count_is_not_scan(monkeypatch):
    pytest.importorskip("pyiceberg")
    pytest.importorskip("pyarrow")
    ...
    mysql = _mysql_connect()
    tag = uuid.uuid4().hex[:8]
    src = f"ice_ice_scan_{tag}"
    mid = f"ice_ice_scan_mid_{tag}"
    dest = f"ice_ice_scan_dst_{tag}"
    try:
        _seed_iceberg_from_mysql(mysql, src, mid, 80)
        _drop_iceberg(dest)

        def _no_count(self):
            raise AssertionError("Iceberg→Iceberg COUNT must not scan().count()")

        def _no_arrow(self):
            raise AssertionError("Iceberg→Iceberg COPY must not scan().to_arrow()")
...
@requires_rest
def test_live_iceberg_iceberg_stream_load_method(monkeypatch):
    pytest.importorskip("pyiceberg")
    pytest.importorskip("pyarrow")
    monkeypatch.delenv("DATAFLOW_ICEBERG_ICEBERG_COPY", raising=False)
    mysql = _mysql_connect()
    tag = uuid.uuid4().hex[:8]
    src = f"ice_ice_str_{tag}"
    mid = f"ice_ice_str_mid_{tag}"
    dest = f"ice_ice_str_dst_{tag}"
    try:
        _seed_iceberg_from_mysql(mysql, src, mid, 800)
        _drop_iceberg(dest)
        ...
        job_id = f"ice-ice-copy-{tag}"
        get_mongodb_service().create_transfer_job({"_id": job_id, "name": job_id})
        source = EndpointConfig.from_dict(
            "database", {**_iceberg_cfg(mid), "format": "iceberg"}
        )
        ...
        assert summary.get("load_method") == "iceberg_snapshot_parquet_cow_iceberg"
