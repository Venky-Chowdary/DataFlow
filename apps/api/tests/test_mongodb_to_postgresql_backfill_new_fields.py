"""End-to-end MongoDB → PostgreSQL schema drift with backfill_new_fields."""
...
def test_mongodb_to_postgresql_backfill_new_fields():
    try:
    ...
    dst_table = f"orders_mongo_backfill_{uuid.uuid4().hex[:8]}"
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute("SELECT column_name FROM information_schema.columns WHERE table_name = %s ORDER BY ordinal_position", (dst_table,))
            cols = [r[0] for r in cur.fetchall()]
        assert "currency" not in cols, cols

        for i in range(1, 3):
            src.replace_one({"order_id": i}, {
                "order_id": i,
                "amount": Decimal128(Decimal(f"{i}00.50")),
        ...
        assert result2.records_transferred == 4

        with conn.cursor() as cur:
            cur.execute("SELECT column_name FROM information_schema.columns WHERE table_name = %s ORDER BY ordinal_position", (dst_table,))
            cols = [r[0] for r in cur.fetchall()]
            cur.execute(f'SELECT order_id, amount, currency FROM public."{dst_table}" ORDER BY order_id')
            rows = cur.fetchall()
        conn.close()

        assert "currency" in cols, cols
        assert rows == [(1, Decimal("100.50"), "USD"), (2, Decimal("200.50"), "EUR"), (3, Decimal("300.50"), "GBP"), (4, Decimal("400.50"), "GBP")]
    finally:
