"""End-to-end execute_tracked CSV→MySQL/MariaDB upsert with reconciliation."""
...
def test_csv_to_mariadb_upsert_updates_and_appends():
    try:
        with socket.create_connection(("localhost", 3307), timeout=1):
            pass
    except OSError:
        pytest.skip("MariaDB emulator not reachable on localhost:3307")
    ...
    conn = pymysql.connect(
        host="localhost", port=3307, database="dataflow",
        user="dataflow", password="dataflow",
    )
    try:
        with conn.cursor() as cur:
            cur.execute('SELECT id, amount FROM payments_upsert_test_e2e ORDER BY id')
            rows = cur.fetchall()
    finally:
        conn.close()
    assert len(rows) == 3
