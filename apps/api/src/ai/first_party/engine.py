...
_SUBJECT_FAMILIES: tuple[frozenset[str], ...] = (
    ...
    frozenset({"google_photos"}),
    frozenset({"google_contacts"}),
    frozenset({"google_maps"}),
    frozenset({"google_news"}),
    frozenset({"google_play"}),
    frozenset({"vision_ai"}),
    frozenset({"speech_to_text"}),
    frozenset({"earth_engine"}),
    frozenset({"bigquery_bi_engine"}),
    frozenset({"azure_media"}),
    frozenset({"azure_cognitive"}),
    frozenset({"azure_bot"}),
    frozenset({"confidential_ledger"}),
    frozenset({"operator_nexus"}),
    frozenset({"microsoft_clarity"}),
    frozenset({"anthos"}),
    frozenset({"iot_hub"}),
    frozenset({"merchant_center"}),
    frozenset({"google_voice"}),
    frozenset({"google_pay"}),
    frozenset({"google_optimize"}),
    frozenset({"translation_api"}),
    frozenset({"recommendations_ai"}),
    frozenset({"vertex_ai_workbench"}),
    frozenset({"azure_health_data"}),
    frozenset({"azure_video_indexer"}),
    frozenset({"cloud_ids"}),
    frozenset({"cloud_deploy"}),
    frozenset({"cloud_source_repos"}),
    frozenset({"cloud_workstations"}),
    frozenset({"artifact_analysis"}),
    frozenset({"confidential_vm"}),
    frozenset({"azure_stack_hub"}),
    frozenset({"azure_stack_hci"}),
    frozenset({"dicom"}),
    frozenset({"microsoft_copilot"}),
    frozenset({"backup_for_gke"}),
    frozenset({"connected_sheets"}),
    frozenset({"looker_embedded"}),
    frozenset({"colab_enterprise"}),
    frozenset({"automl"}),
    frozenset({"digital_twins"}),
    frozenset({"spatial_anchors"}),
    frozenset({"floodlight"}),
    frozenset({"beyondcorp"}),
    frozenset({"tekton"}),
    frozenset({"traffic_director"}),
    frozenset({"parallelstore"}),
    frozenset({"netapp_volumes"}),
    frozenset({"adls"}),
